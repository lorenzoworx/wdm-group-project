...
// Similar style to client/src/api/classes.js
...
export async function getExams() {
    const resp = await fetch(`${BASE}/api/exams/list.php`, {
        headers: { Accept: 'application/json' },
    });
    const body = await parseJson(resp);
    if (!resp.ok) throw new Error(body.error || 'Failed to load exams');
    return body; // { exams: [...] }
}
...
export async function createExam(payload) {
    ...
    const resp = await fetch(`${BASE}/api/exams/create.php`, {
        method: 'POST', headers, body: JSON.stringify(payloadWithToken)
    });
    const body = await parseJson(resp);
    if (!resp.ok) throw new Error(body.error || `HTTP ${resp.status}`);
    return body; // { ok: true, exam }
}
...
export async function updateExam(payload) {
    ...
    const payloadWithToken = { ...payload, token };
    const resp = await fetch(`${BASE}/api/exams/update.php`, {
        method: 'POST', headers, body: JSON.stringify(payloadWithToken)
    });
    const body = await parseJson(resp);
    if (!resp.ok) throw new Error(body.error || `HTTP ${resp.status}`);
...
export async function deleteExam(id) {
    ...
    const resp = await fetch(`${BASE}/api/exams/delete.php`, {
