...
const UsersPage = () => {
  ...
  const loadUsers = () => {
    ...
    const hasAdmin = allUsers.some(u => u.email === 'admin@uta.edu');
    if (!hasAdmin) {
      allUsers.push({
        id: 'admin',
        name: 'Admin',
  ...
  const handleCreateUser = (e) => {
    ...
    if (userToAdd.userType === 'admin' || userToAdd.userType === 'instructor') {
      const adminUsers = JSON.parse(localStorage.getItem('adminUsers') || '[]');
  ...
  const handleUpdateUser = (userId, updates) => {
    ...
    const adminUsers = updatedUsers.filter(u => u.userType === 'admin' || u.userType === 'instructor');
    const localUsers = updatedUsers.filter(u => u.userType === 'student' || u.userType === 'qa');
